<template>
  <div class="main-content">
      ...
      <div class="table-content">
        <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
            ...
            <el-table-column width="300" :align="contents.tableAlign"
                ...
                <el-button v-if="isAuth('yuangong','加入通讯录') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 1" type="success" icon="el-icon-tickets" size="mini" @click="renyuantongxunluCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'加入通讯录':'' }}</el-button>
                <el-button v-if="isAuth('yuangong','加入通讯录') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 2" type="success" size="mini" @click="renyuantongxunluCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'加入通讯录':'' }}<i class="el-icon-tickets el-icon--right" /></el-button>
                <el-button v-if="isAuth('yuangong','加入通讯录') && contents.tableBtnIcon == 0" type="success" size="mini" @click="renyuantongxunluCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'加入通讯录':'' }}</el-button>
    ...
    <renyuantongxunlu-cross-add-or-update v-if="renyuantongxunluCrossAddOrUpdateFlag" :parent="this" ref="renyuantongxunluCrossaddOrUpdate"></renyuantongxunlu-cross-add-or-update>
...
import renyuantongxunluCrossAddOrUpdate from "../renyuantongxunlu/add-or-update";
export default {
  data() {
    return {
      searchForm: {
      ...
      renyuantongxunluCrossAddOrUpdateFlag: false,
  ...
  components: {
    AddOrUpdate,
    renyuantongxunluCrossAddOrUpdate,
  },
  methods: {
    contentStyleChange() {
    ...
    renyuantongxunluCrossAddOrUpdateHandler(row,type){
      this.showFlag = false;
      this.addOrUpdateFlag = false;
      this.renyuantongxunluCrossAddOrUpdateFlag = true;
      this.$storage.set('crossObj',row);
      this.$storage.set('crossTable','yuangong');
      this.$nextTick(() => {
      this.$refs.renyuantongxunluCrossaddOrUpdate.init(row.id,type);
      });
    },
    init () {
    },
